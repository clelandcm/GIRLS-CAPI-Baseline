...
options(width = 150)
knitr::opts_chunk$set(echo = FALSE, message=FALSE, warning=FALSE, comment=NA, fig.width = 6, fig.asp = 0.618, out.width = "80%", fig.align = "center", dpi=200, dev = "svg")
```
...
```{r}
most.recent.data.subfolder <- "C:/chuck/NYU/Kurth/High Yield Testing/Data/July 2018"
```

```{r}
# Read in CSV Data

capi.files <- list.files(most.recent.data.subfolder, pattern = "BASELINE_BEHAVIORAL_SURVEY___GIRLS____", full.names = TRUE)
selftest.files <- list.files(most.recent.data.subfolder, pattern = "SELF_TESTING_SURVEY", full.names = TRUE)
negcohort.files <- list.files(most.recent.data.subfolder, pattern = "HIV_Negative_Cohort", full.names = TRUE)
poscohort.files <- list.files(most.recent.data.subfolder, pattern = "HIV_Positive_Cohort", full.names = TRUE)
genelg.files <- list.files(most.recent.data.subfolder, pattern = "General_Eligibility_Criteria", full.names = TRUE)
htcreg.files <- list.files(most.recent.data.subfolder, pattern = "Electronic_HTC_Register", full.names = TRUE)
repcap <- read_csv(paste(most.recent.data.subfolder, 
                         "01___BASELINE_BEHAVIORAL_SURVEY___GIRLS__REPEAT_CLIENTS____20th_June_2017_results.csv", sep="/")
                   )

cap <- Reduce(function(x, y) merge(x, y, all=TRUE), lapply(capi.files, read_csv))
elg <- Reduce(function(x, y) merge(x, y, all=TRUE), lapply(genelg.files, read_csv))
htc <- Reduce(function(x, y) merge(x, y, all=TRUE), lapply(htcreg.files, read_csv))
neg <- Reduce(function(x, y) merge(x, y, all=TRUE), lapply(negcohort.files, read_csv))
pos <- Reduce(function(x, y) merge(x, y, all=TRUE), lapply(poscohort.files, read_csv))
slf <- Reduce(function(x, y) merge(x, y, all=TRUE), lapply(selftest.files, read_csv))  

names(cap) <- gsub(":", "_", names(cap))
names(elg) <- gsub(":", "_", names(elg))
names(htc) <- gsub(":", "_", names(htc))
names(neg) <- gsub(":", "_", names(neg))
names(pos) <- gsub(":", "_", names(pos))
names(slf) <- gsub(":", "_", names(slf))

enrolled_ID <- read.xlsx("C:/chuck/NYU/Kurth/High Yield Testing/Data/July 2018/PID Enrollment Update 10APR2018.xlsx", sheet = 4)

names(enrolled_ID)[c(3,5)] <- c("PUID","Status")
```

# Fixing Errors
```{r}
Q37_errors <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/Error Log/GIRLS Error Tracking Log12JUL2018.xlsx", 
                        sheet = "Q37", rows = 1:51)

withdrew <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/Error Log/GIRLS Error Tracking Log12JUL2018.xlsx", 
                        sheet = "Withdrew")

LTF <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/Error Log/GIRLS Error Tracking Log12JUL2018.xlsx", 
                        sheet = "Lost to Follow-up")

Excluded <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/Error Log/GIRLS Error Tracking Log12JUL2018.xlsx", 
                        sheet = "Excluded")

Self_Testing_Survey <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/Error Log/GIRLS Error Tracking Log12JUL2018.xlsx", 
                        sheet = "Self-testing Survey")

capi_errors <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/Error Log/GIRLS Error Tracking Log12JUL2018.xlsx", 
                        sheet = "Baseline CAPI")

Misc_errors <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/Error Log/GIRLS Error Tracking Log12JUL2018.xlsx", 
                        sheet = "Miscellaneous")

cap$PUID[cap$PUID == "09-0800-01-01"] <- "09-0800-01-03"
cap$PUID[cap$PUID == "18-0984-01-02" & cap$date > "2018-02-20"] <- "18-1084-01-02"
cap$PUID[cap$PUID == "O8-0365-01-01"] <- "08-0365-01-01"
cap$PUID[cap$PUID == "17--0303-01-01"] <-  "17-0303-01-01"
cap$PUID[cap$PUID == "17-305-02-01"] <-    "17-0305-02-01"
cap$PUID[cap$PUID == "06-0646-01-3"] <-    "06-0646-01-03"
cap$PUID[cap$PUID == "17-  0624-01-03"] <- "17-0624-01-03"
cap$PUID[cap$PUID == "17-0748--01-03"] <-  "17-0748-01-03"
cap$PUID[cap$PUID == "06-0951-01-02"] <-   "06-0952-01-02"
cap$PUID[cap$PUID == "06-0950-01-02" & cap$Q2 == 18] <-   "06-0951-01-02"
cap$PUID[cap$PUID == "12-0046-01-01" & cap$Q2 == 23] <- "12-0047-01-01"
cap$PUID[cap$PUID == "06-0316-02-01"] <- "06-0316-01-01"
cap$PUID[cap$PUID == "17-0307-02-01"] <- "17-0307-01-01"
cap$Venue[cap$PUID == "17-0361-02-01"] <- 2
cap$Venue[cap$PUID == "06-0147-02-01"] <- 2
cap$Venue[cap$PUID == "09-1211-02-02"] <- 2
cap$Venue[cap$PUID == "06-1207-02-02"] <- 2
cap$Venue[cap$PUID == "18-1140-02-02"] <- 2
cap$Venue[cap$PUID == "06-0515-01-01"] <- 1
cap$PUID[cap$PUID == "09-0804-01-01"] <- "09-0804-01-03"
cap$PUID[cap$PUID == "09-0805-01-01"] <- "09-0805-01-03"
cap$PUID[cap$PUID == "09-0807-01-01"] <- "09-0807-01-03"
cap$PUID[cap$PUID == "09-0809-01-01"] <- "09-0809-01-03"
cap$Q96[cap$PUID == "08-0032-01-01"] <- 2
cap$Q96[cap$PUID == "06-0099-02-01"] <- 2
cap$Q96[cap$PUID == "06-0255-01-01"] <- 2
cap$Q96[cap$PUID == "09-0330-01-01"] <-	2
cap$Q96[cap$PUID == "10-0290-01-01"] <-	2
cap$Q96[cap$PUID == "08-0370-01-01"] <-	2
cap$Q96[cap$PUID == "06-0954-01-02"] <-	2
cap$Q96[cap$PUID == "18-0987-01-02"] <-	2
cap$Q96[cap$PUID == "05-1079-01-02"] <-	2
cap$Q96[cap$PUID == "06-1103-01-02"] <-	2
cap$Q96[cap$PUID == "18-1148-01-02"] <-	2
cap$Q96[cap$PUID == "18-1090-01-02"] <-	2
cap$Q96[cap$PUID == "07-0191-02-01"] <- 1
cap$Q96[cap$PUID == "06-0515-01-01"] <-	1
cap$Q96[cap$PUID == "09-0002-01-01"] <-	1
cap$Q96[cap$PUID == "08-0029-01-01"] <-	1
cap$Q96[cap$PUID == "08-0035-01-01"] <-	1


repcap$PUID[repcap$PUID == "06-0024-01-01"] <- "08-0024-01-01"

slf$PUID[slf$PUID == "17-0307-02-01"] <- "17-0307-01-01"
slf$Venue[slf$PUID == "06-0249-01-01"] <- 1
slf$Venue[slf$PUID == "10-0274-02-01"] <- 2
slf$Venue[slf$PUID == "08-0271-02-01"] <- 2

cap$Grp7_Q37[cap$PUID %in% Q37_errors$PTID] <- 3

elg$PUID[elg$PUID == "051008-01-02"] <-   "05-1008-01-02"
elg$PUID[elg$PUID == "09-1033-0102"] <-   "09-1033-01-02"
elg$PUID[elg$PUID == "09-1046-01-2"] <-   "09-1046-01-02"
elg$PUID[elg$PUID == "17--0303-01-01"] <- "17-0303-01-01"
elg$PUID[elg$PUID == "17-0748--01-03"] <- "17-0748-01-03"
elg$PUID[elg$PUID == "18- 0985-01-02"] <- "18-0985-01-02"
htc$group3_PTID[htc$group3_PTID == "09-1046-01-2"] <-   "09-1046-01-02"
neg$PUID[neg$PUID == "09-1046-01-2"] <-   "09-1046-01-02"

DF <- as.data.table(cap)  
```
...
Excluded_IDs <- Excluded$IDNumber

DF <- DF %>% dplyr::filter(!(PUID %in% Excluded_IDs))
HTC <- HTC %>% filter(!(PUID %in% Excluded_IDs))
REPEATs <- REPEATs %>% filter(!(PUID %in% Excluded_IDs))

all.ID <- unique(c(DF$PUID, REPEATs$PUID, HTC$PTID, ELG$PUID, neg$PUID, pos$PUID, slf$PUID))

tracker <- data.frame(PUID = all.ID, ELG = all.ID %in% ELG$PUID, CAPI1 = all.ID %in% DF$PUID, 
                      CAPI2 = all.ID %in% REPEATs$PUID, HTC = all.ID %in% HTC$PUID,
                      NEG = all.ID %in% neg$PUID, POS = all.ID %in% pos$PUID, SLF = all.ID %in% slf$PUID)
```
...
```{r}
tracker %>% 
  select(PUID, ELG, CAPI1, HTC, NEG, POS, SLF) %>% 
  filter(nchar(as.character(PUID)) != 13) %>% 
  arrange(PUID, ELG, CAPI1, HTC) %>%
  knitr::kable()
```

* 03___SELF_TESTING_SURVEY____6th_February_2018_results.csv (Row 33)
...
DF %>% 
  ...
  select(PUID, date, Q1, Q2, InterNo, meta_instanceID, start, end) %>%
  knitr::kable()
} else{
...
REPEATs$Grp7_Grp8z_Q44 <- as.character(REPEATs$Grp7_Grp8z_Q44)
REPEATs$Grp7_Q49 <- as.character(REPEATs$Grp7_Q49)
REPEATs$Grp7_Q53a <- as.character(REPEATs$Grp7_Q53a)

# Add Items from First Interview to REPEATs 
REPEATs <- left_join(REPEATs[, colSums(!is.na(REPEATs)) > 0], 
                     DF %>% 
                       select(PUID, Q3, Q4, Q5, Q5a, Q6, Q7, CountyLive, SubCountyLive, DivisionLive,
...
DF %>%
  ...
  scale_x_continuous(limits = c(14.5,25.5), breaks = seq(15,24,1)) +
  theme_minimal() 
```
...
DF <- DF %>%
  mutate(Q18 = as.numeric(Q18),
         Q18 = car::recode(Q18, "0=0;1=1;2=2;3=3;4=4;else=0"),
         Q19 = car::recode(Q19, "0=0;1=1;2=2;3=3;4=4;else=0"),
         Q20 = car::recode(Q20, "0=0;1=1;2=2;3=3;4=4;else=0")) %>%
  rowwise() %>%
  mutate(AUDITC_Score = sum(c(Q18,Q19,Q20)),
         AUDITC_Positive = ifelse(AUDITC_Score >= 3, "Yes", "No")) %>%
  ungroup()

DF %>%
  count(AUDITC_Positive) %>%
  mutate(Percent = round(prop.table(n) * 100, 1)) %>%
  knitr::kable()
```
...
```{r}
DF <- DF %>%
  mutate(Q25 = car::recode(Q25, "0=0;1=1;2=2;3=3;else=0"),
         Q26 = car::recode(Q26, "0=0;1=1;2=2;3=3;else=0")) %>%
  rowwise() %>%
  mutate(PHQ2_Score = sum(c(Q25,Q26)),
         PHQ2_Positive = ifelse(PHQ2_Score >= 3, "Yes", "No")) %>%
  ungroup()
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q28), " ")
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q63), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q65), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Grp130_Grp131_Q72), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Grp130_Grp131_Q76), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Grp130_Grp131_Q77), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q83), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q84), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q86), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q88), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q93), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
```{r}
Multiple.Selection <- strsplit(as.character(DF$Q95), " ")
Multiple.Selection <- setNames(Multiple.Selection, seq_along(Multiple.Selection))
Multiple.Selection <- stack(Multiple.Selection)
Multiple.Selection <- filter(Multiple.Selection, !is.na(values))
...
* The HTC data has duplicated ID numbers and ID numbers that don't seem to match any in CAPI baseline.

# High Risk Negative
```{r}
neg_cohort_XL <- read.xlsx("c:/chuck/NYU/Kurth/High Yield Testing/Data/July 2018/Negative Cohort.xlsx", detectDates = TRUE)

DF <- DF %>%
  mutate(Positive_Partners = !is.na(Grp7_Q53a) & Grp7_Q53a != "0",
         Unknown_Partners = !is.na(Grp7_Q53d) & Grp7_Q53d != "0",
         Multiple_Partners = !is.na(Grp7_Grp8z_Q44a) & Grp7_Grp8z_Q44a >= 3,
         Partner_Multiple_Partners = !is.na(Grp7_Q44bb) & Grp7_Q44bb == 1,
         HRN_Flagged = !is.na(Grp19x_HRiskNeg) & Grp19x_HRiskNeg == "OK",
         HRN_Enrolled = PUID %in% neg_cohort_XL$PTID,
         High_Risk_Negative = case_when(Q31 == 1 &
                                        (Positive_Partners |
                                         Unknown_Partners |
                                         Multiple_Partners |
                                         Partner_Multiple_Partners) ~ "Yes",
                                        TRUE ~ "No"))

DF %>%
  count(High_Risk_Negative) %>%
  mutate(Percent = round(prop.table(n) * 100, 1)) %>%
  arrange(desc(n)) %>%
  knitr::kable()
```
...
```{r, eval=FALSE}
setdiff(DF$PUID, enrolled_ID$PUID[enrolled_ID$Status == "On Study"])
setdiff(enrolled_ID$PUID[enrolled_ID$Status == "On Study"], DF$PUID)
```

```{r, eval=FALSE}
file.rename(from = "C:/chuck/NYU/Kurth/High Yield Testing/R Code/CAPI-Summaries-16July2018.html", 
