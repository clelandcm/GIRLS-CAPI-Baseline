...
Girls.CSV.files <- list.files(path='c:/chuck/NYU/Kurth/High Yield Testing/Data/', pattern = ".csv$|.CSV$", full.names = TRUE)
Girls.CSV.list <- vector(mode="list", length = length(Girls.CSV.files))
for(i in 1:length(Girls.CSV.list[1:2])){Girls.CSV.list[[i]] <- read_csv(file = Girls.CSV.files[i], col_types = cols(`Grp7:Q36a` = 'i',
                                                                                                               `Grp7:Q53` = 'c',
                                                                                                               `Grp7:Q53b` = 'i',
                                                                                                               `Grp7:Q53d` = 'i',
                                                                                                               `Grp7:Q53e` = 'i',
                                                                                                               ...
                                                                                                               `Q93` = 'c',
                                                                                                               `Q95` = 'c'))}
names(Girls.CSV.list) <- Girls.CSV.files

DF <- bind_rows(Girls.CSV.list[1:2], .id = "file_from")
names(DF) <- gsub(":", "_", names(DF))

DF <- as.data.table(DF)

REPEATs <- read_csv(Girls.CSV.files[3])

names(REPEATs) <- gsub(":", "_", names(REPEATs))

HTC <- bind_rows(read_csv(Girls.CSV.files[5]), read_csv(Girls.CSV.files[4]))
HTC <- as.data.table(HTC)
names(HTC) <- gsub("group2:|group1:|group3:|group5:", "", names(HTC))
names(HTC) <- gsub(":", "_", names(HTC))

all.ID <- unique(c(DF$PUID, REPEATs$PUID, HTC$PTID))

tracker <- data.frame(PUID = all.ID, CAPI1 = all.ID %in% DF$PUID, CAPI2 = all.ID %in% REPEATs$PUID, HTC = all.ID %in% HTC$PTID)
```
...
## ID Numbers Appearing Only in Repeats File
```{r}
tracker %>% 
  filter(!CAPI1 & CAPI2) %>%
  select(PUID, CAPI1, CAPI2) %>%
  knitr::kable()
```
...
DF %>% 
  ...
  mutate(Item = gsub("Grp13_Q66","", Item),
         ...
         Response = case_when(Response == "1" ~ "Yes",
                              ...
                              Response == "99" ~ "99")) %>%
  group_by(Item) %>%
  summarize(`No n` = sum(Response == 'No'),
            `No %` = round(mean(Response == 'No') * 100, 1),
            `Yes n` = sum(Response == 'Yes'),
            `Yes %` = round(mean(Response == 'Yes') * 100, 1)) %>%
