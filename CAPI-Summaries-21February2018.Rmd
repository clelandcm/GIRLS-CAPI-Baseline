...
source("c:/chuck/NYU/Kurth/High Yield Testing/R Code/capi-column-types.r")
```

```{r}
capi.list <- lapply(capi.files, function(x){read_csv(file = x, col_types = capi.cols)})
selftest.list <- lapply(selftest.files, function(x){read_csv(file = x)})
negcohort.list <- lapply(negcohort.files, function(x){read_csv(file = x)})
poscohort.list <- lapply(poscohort.files, function(x){read_csv(file = x)})
genelg.list <- lapply(genelg.files, function(x){read_csv(file = x)})
htcreg.list <- lapply(htcreg.files, function(x){read_csv(file = x, col_types = cols(`Couple` ='i'))})
...
Excluded_IDs <- c('08-0030-01-01','10-0584-01-01','06-0064-01-01','08-0112-01-01',
                  '06-0103-02-01','10-0089-02-01','07-0186-02-01','10-0277-02-01',
                  '08-0269-02-01','06-0396-02-01','08-0469-01-01','06-0251-02-01',
                  '17-0451-01-01','09-0010-01-01','10-0283-02-01')

# Rao email on July 13, 2017 and Nok Chhun emails on August 21, 2017 and September 13, 2017
DF <- DF %>% dplyr::filter(!(PUID %in% Excluded_IDs) & date <= "2018-02-21" & SubCountyz != "Ndhiwa")
HTC <- HTC %>% filter(!(PTID %in% Excluded_IDs) & today <= "2018-02-21")
...
```{r, eval=FALSE}
file.rename(from = "C:/chuck/NYU/Kurth/High Yield Testing/R Code/CAPI-Summaries-21February2018.html", 
