...
```{r}
girls.dirs <- file.info(list.dirs("C:/Bsync/3. Server Backup Files"))
most.recent.data.subfolder <- tail(rownames(girls.dirs[order(girls.dirs$ctime),]), n=1)
```
...
capi.list <- vector(mode="list", length = length(capi.files))
selftest.list <- vector(mode="list", length = length(selftest.files))   
negcohort.list <- vector(mode="list", length = length(negcohort.files))
poscohort.list <- vector(mode="list", length = length(poscohort.files))
genelg.list <- vector(mode="list", length = length(genelg.files))
htcreg.list <- vector(mode="list", length = length(htcreg.files))

load("c:/chuck/NYU/Kurth/High Yield Testing/Data/ctypes.RData", .GlobalEnv)
# capi.cols <- car::recode(BL.Capi.Col.Types, "'character'='c';'integer'='i';'double'='d'", as.factor.result = FALSE)
# names(capi.cols) <- names(read_csv(capi.files[[1]]))

for(i in 1:length(capi.list)){capi.list[[i]] <- read_csv(file = capi.files[i], col_types = cols(`Grp7:Q49` = 'i',
                                                                                                `Grp7:Q53e` = 'i',
                                                                                                `Grp130:Grp131:Q77` = 'i',
                                                                                                `Q93` = 'i',
                                                                                                `Grp7:Grp8z:Q44` = 'i',
                                                                                                ...
                                                                                                `Grp7:Grp8z:Q44c2` = 'i',
                                                                                                `Grp7:Grp8z:Q44c1` = 'i'))}
for(i in 1:length(selftest.list)){selftest.list[[i]] <- read_csv(file = selftest.files[i])}
for(i in 1:length(negcohort.list)){negcohort.list[[i]] <- read_csv(file = negcohort.files[i])}
for(i in 1:length(poscohort.list)){poscohort.list[[i]] <- read_csv(file = poscohort.files[i])}
for(i in 1:length(genelg.list)){genelg.list[[i]] <- read_csv(file = genelg.files[i])}
for(i in 1:length(htcreg.list)){htcreg.list[[i]] <- read_csv(file = htcreg.files[i], col_types = cols(`Couple` ='i'))}

names(capi.list) <- capi.files
names(selftest.list) <- selftest.files
names(negcohort.list) <- negcohort.files
names(poscohort.list) <- poscohort.files
names(genelg.list) <- genelg.files
names(htcreg.list) <- htcreg.files

htc <- bind_rows(htcreg.list, .id = "file_from")
elg <- bind_rows(genelg.list, .id = "file_from")
neg <- bind_rows(negcohort.list, .id = "file_from")
pos <- bind_rows(poscohort.list, .id = "file_from")
slf <- bind_rows(selftest.list, .id = "file_from")
cap <- bind_rows(capi.list, .id = "file_from")
...
Excluded_IDs <- c('08-0030-01-01','06-0064-01-01','08-0112-01-01','06-0103-02-01',
                  '10-0089-02-01','07-0186-02-01','10-0277-02-01','08-0269-02-01',
                  '06-0396-02-01','08-0469-01-01','06-0251-02-01','17-0451-01-01')
                  
# Rao email on July 13, 2017 and Nok Chhun emails on August 21, 2017 and September 13, 2017
DF <- DF %>% filter(!(PUID %in% Excluded_IDs) & date <= "2017-12-19")
HTC <- HTC %>% filter(!(PTID %in% Excluded_IDs) & today <= "2017-12-19")
...
tracker <- data.frame(PUID = all.ID, ELG = all.ID %in% ELG$PUID, CAPI1 = all.ID %in% DF$PUID, 
                      CAPI2 = all.ID %in% REPEATs$PUID, HTC = all.ID %in% HTC$PTID,
...
```{r, eval=FALSE}
file.rename(from = "C:/chuck/NYU/Kurth/High Yield Testing/R Code/CAPI-Summaries-4January2018.html", 
